...
def demo_JmE(X, Y, DPiters=1000):
    ...
    model = MixtureOfExperts.models.JointMixtureOfExperts(X, Y, expert, mixedLikelihood, probit=probit,
                                                          name='JmE_adni_initbaseline', mneal8=3,
                                                          initialisation=X[:, -1])
    print(model)
...
def demo_pull_information(model, root, burnin=150):
    ...
    with open(root + 'JmE/' + model.__name__ + "_feature.csv", "wb") as f:
...
if __name__ == "__main__":
    ...
    logging.basicConfig(format='%(asctime)s: %(levelname)s: %(module)s: %(message)s',
                        datefmt='%m/%d/%Y %I:%M:%S %p', level=logging.DEBUG)  #filename='adniJ.log',
    ...
    for _ in range(1):
        model = demo_JmE(Xtrain, Ytrain, DPiters=10)
